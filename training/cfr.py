...
class CFR:
    ...
    def generate_model(self):
        ...
        return DeepCFRModel(4, self.nbets, self.nactions).to(torch.device("cuda"))
    ...
    def infoset_EV(self, roundstate, traverser, t):
        ...
        pnls = torch.zeros(2, device=torch.device("cuda"))

        for i in range(self.mcc_iters):

            round_pnls = self.game_engine.simulate_round_state(players, roundstate)
            round_pnls = torch.tensor(round_pnls, device=torch.device("cuda"))
            pnls += round_pnls
    ...
    def regret_tensor(self, roundstate, traverser, t):
        ...
        regret_tensor = torch.zeros(5, device=torch.device("cuda"))
