...
class DeepCFRModel(nn.Module):
    ...
    def get_card_num(self, card):
        ...
        return torch.tensor([card_num], device=torch.device("cuda"))

    def tensorize_cards(self, my_hand, board):

        hole_tensor = torch.tensor([self.get_card_num(card) for card in my_hand], device=torch.device("cuda")).unsqueeze(0)
        ...
        flop_tensor = torch.tensor(board_nums[:3], device=torch.device("cuda")).unsqueeze(0)
        turn_tensor = torch.tensor([board_nums[3]], device=torch.device("cuda")).unsqueeze(0)
        river_tensor = torch.tensor([board_nums[4]], device=torch.device("cuda")).unsqueeze(0)
    ...
    def tensorize_bets(self, bets):
        ...
        tensorized_bets = torch.tensor(last_bets, dtype=torch.float32, device=torch.device("cuda")).unsqueeze(0)
    ...
    def tensorize_mask(self, round_state):
        ...
        mask_tensor = torch.zeros(self.nactions, dtype=torch.float32, device=torch.device("cuda"))
